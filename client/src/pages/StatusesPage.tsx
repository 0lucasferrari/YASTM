...
import { statuses } from '../services/api.ts';
...
export default function StatusesPage() {
  ...
  const load = async () => {
    try {
      const data = await statuses.list();
      setItems(data as Status[]);
    } catch {
      setError(t('statuses.loadFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await statuses.update(editing.id, body);
      } else {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">{t('statuses.title')}</Typography>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editing ? t('statuses.editStatus') : t('statuses.newStatus')}</DialogTitle>
        <DialogContent>
          {error && dialogOpen && (
